<template>
    <div>
        ...
        <el-button type="primary" @click="asc();is='asc'">升序查询</el-button>
        <el-button type="primary" @click="desc();is='desc'">降序查询</el-button>
    ...
    <div>
        ...
        <div v-if="datas.total" style="margin-top: 20px;">
            <el-pagination background layout="prev, pager, next" @change="is=='asc'?asc():desc()" v-model:current-page="datas.pageNum" v-model:page-size="datas.pageSize" :total="datas.total" />
        </div>
...
const insert = () => {
  request.post('/time/insert',datas.form).then(result => {
     if(result.code === 200){
      ElNotification({
        title: '提示',
        message: '添加成功',
...
const renew = () => {
    request.put('/time/update',datas.form).then(result => {
      if(result.code === 200){
        ElNotification({
          title: '提示',
          message: '修改成功',
...
const cancel = (row) => {
  ElMessageBox.confirm(
    ...
    .then(() => {
      request.delete('/time/delete/' + row.id).then(result => {
        if(result.code === 200){
          ElNotification({
            title: '提示',
            message: '删除成功',
